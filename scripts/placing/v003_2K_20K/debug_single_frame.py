...
def debug_single_train_scene():
    ...
    new_train_path = "/home/mguo/debug_single_frame/new_train.png"
    ...
    test_path = test_correct_path

    train_img_original = imageio.imread(train_path)
    test_img_original = imageio.imread(test_path)

    train_img = train_img_original.copy().astype(np.float32)
    test_img = test_img_original.copy().astype(np.float32)
    ...
    for i in range(H):
        for j in range(W):
            train_pixel = list(train_img[i, j])
            test_pixel = list(test_img[i, j])
    ...
    seg_img = imageio.imread("/home/mguo/train_masks.png")
    ...
    # Another thing to try: the newly trained placing vision model.
